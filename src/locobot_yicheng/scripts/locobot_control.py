...
class sendvelocity(Node):
    def __init__(self,name):
        ...
        self.kp_values = [10]
        self.current_kp = self.kp_values[0] #initialize the first one
        # self.current_kp = 0.5 #initialize the first one

        # prepare for the plots
        self.data_collections = {kp: {'time': [], 'x': [], 'y': [], 'rx': [], 'ry': []} for kp in self.kp_values}
        # self.data_collections = {'time': [], 'x': [], 'y': [], 'rx': [], 'ry': []}
...
# a function that saves the data to a csv file'File 
def save_data(data_collections):
    for kp in data_collections.keys():
        data = np.array([data_collections[kp]['time'], data_collections[kp]['x'], data_collections[kp]['y'], data_collections[kp]['rx'], data_collections[kp]['ry']])
        np.savetxt(f'data_kp_{kp}.csv', data, delimiter=',')
    print('Data saved to CSV files',f'data_kp_{kp}.csv')
...
def main():
    ...
    try:
        ...
        save_data(node.data_collections)
    finally:
        rclpy.shutdown()
